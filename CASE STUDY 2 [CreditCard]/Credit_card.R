...
summary(cust_acq$Age) # No age value below 18
...
Top_10_Customers <- modified_repay_data %>%
                      group_by(Customer) %>% 
                      summarise(max_repay=sum(Amount)) %>% 
                      arrange(desc(max_repay)) %>% head(10)
...
# Use of the function:
top_cust(product = "Silver",time = "Year")
top_cust("Gold","Month")
