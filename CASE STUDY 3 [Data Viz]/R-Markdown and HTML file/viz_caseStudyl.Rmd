...
Store_data_mod <-  Store_data %>%
                    pivot_longer(-(1:8), names_to = c("measure", "year"),names_sep = -4) %>%
                    group_by(measure) %>%
...
```{r message=F,tidy=TRUE,fig.height=6.5,fig.align='center'}
Ans1 <-   Store_data_mod %>%
...
plotrix::pie3D(region_wise$total,
...
```{r message=F,tidy=TRUE,fig.height=6.5,fig.width=10,fig.align='center'}
Ans4 <- Store_data_mod %>%
        filter(Region == "East") %>%
        group_by(State, year) %>%
        summarise(total = sum(Sales)) %>%
        ...
        scale_fill_manual(values = c("tomato","steelblue")) +
        theme_bw() +
        labs(title = "East Region Sales",subtitle = "2015-2016",y = "Total Sales",fill = "Year",
...
Store_data_mod$Qtr <- NA # Creating balnk column
Store_data_mod$Qtr[Store_data_mod$Month %in% c("Jan", "Feb", "Mar")] <- "Q1"
Store_data_mod$Qtr[Store_data_mod$Month %in% c("Apr", "May", "Jun")] <- "Q2"
Store_data_mod$Qtr[Store_data_mod$Month %in% c("Jul", "Aug", "Sep")] <- "Q3"
Store_data_mod$Qtr[Store_data_mod$Month %in% c("Oct", "Nov", "Dec")] <- "Q4"
...
#### Q7 : Compare Qtr wise sales of 2015 and 2016
...
```{r message=F,tidy=TRUE,fig.height=5,fig.width=6,fig.align='center' }
